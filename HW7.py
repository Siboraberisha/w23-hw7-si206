...
# Your student id: 4886 6461 
# Your email: sberisha@umich.edu
# List who you have worked with on this project: alone :(
...
def make_players_table(data, cur, conn):
    # make the players table with all the variables 
    ...
    # find the position for each player
    positions = {} # empty dict
    cur.execute("SELECT id, position FROM Positions")
    for row in cur.fetchall(): # for each row from by the query 
...
def nationality_search(countries, cur, conn):
    # placeholders for the length of list of countriesseparated by commas 
    placeholders = ', '.join(['?'] * len(countries))
    # search for players with nationalities in the given list
...
def birthyear_nationality_search(age, country, cur, conn):
    #calculating the birth year based on desired birth
    desired_birth = 2023-age
    #query to select the desired variables
    ...
    cur.execute(query, (country, desired_birth))
    rows = cur.fetchall()
    return rows
...
def position_birth_search(position, age, cur, conn):
    # calcualte the birthyear based on current year
    year = 2023 - age
    # join players table and positions table 
